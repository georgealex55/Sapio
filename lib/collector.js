...
async function collectRSS(source) {
  ...
  return compact(rssEntries(xml).slice(0,source.limit||12).map(x => obs(source,{...x,publishedAt:x.publishedAt ? new Date(x.publishedAt).toISOString():new Date().toISOString()})));
}
