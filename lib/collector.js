...
const UA = process.env.REDDIT_USER_AGENT || 'web:sapio-cultural-intelligence:v1.0 (by /u/sapio-owner)';
...
async function serialized(adapter,fn,gapMs=450){
  const prior=adapterQueues.get(adapter)||Promise.resolve();
  let release;
  const next=new Promise(resolve=>{release=resolve});
  adapterQueues.set(adapter,prior.then(()=>next));
  await prior;
  try{return await fn()}finally{await sleep(gapMs);release()}
}
...
async function getRedditAccessToken(){
  ...
  if(redditTokenCache&&redditTokenCache.expiresAt>Date.now()+60000)return redditTokenCache.token;
  ...
  if(!data.access_token)throw new Error('Reddit OAuth token missing');
  redditTokenCache={token:data.access_token,expiresAt:Date.now()+Math.max(300,Number(data.expires_in||3600))*1000};
...
async function collectReddit(source) {
  ...
  try {
    ...
    const endpoint=token
      ? `https://oauth.reddit.com/r/${encodeURIComponent(source.subreddit)}/hot?limit=${limit}&raw_json=1`
      : `https://www.reddit.com/r/${encodeURIComponent(source.subreddit)}/hot.json?limit=${limit}&raw_json=1`;
...
async function collectPubMed(source) {
  ...
  const identity=`&tool=${encodeURIComponent(process.env.NCBI_TOOL||'sapio')}&email=${encodeURIComponent(process.env.NCBI_EMAIL||'')}`;
  const search = await fetchJson(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&sort=pub+date&retmax=${limit}&term=${q}${apiKey}${identity}`);
  const ids = search?.esearchresult?.idlist || [];
  if (!ids.length) return [];
  const summary = await fetchJson(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&retmode=json&id=${ids.join(',')}${apiKey}${identity}`);
...
async function collectSource(source) {
  try {
    ...
    else if (source.adapter==='reddit') items=await serialized('reddit',()=>collectReddit(source),900);
    else if (source.adapter==='pubmed') items=await serialized('pubmed',()=>collectPubMed(source),450);
...
function stage(score, sources) {
  ...
  if(score>=80&&sources>=4) return 'BREAKING';
...
function clusterObservations(observations) {
  ...
  return clusters.map(c=>{
    ...
    // Questionnaire v1: discovery, novelty and independent breadth lead; credibility supports rather than dominates.
    const score=Math.round(velocity*.20+novelty*.25+avgEdge*.10+breadth*.25+evidence*.05+culturalReach*.15);
    ...
    return {
      ...
      observationIds:c.observations.map(o=>o.id),
      sourceLinks:c.observations.slice(0,12).map(o=>({sourceId:o.sourceId,source:o.source,publisher:o.publisher||'',role:o.role,url:o.sourceUrl,publishedAt:o.publishedAt}))
    };
...
async function collectAll() {
  ...
  return {
    ...
    run:{
      ...
      sourceRuns:results.map(r=>({source:r.source,ok:r.ok,items:r.items.length,error:r.error||null}))
    },
    items:clusters
  };
}
