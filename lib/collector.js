...
async function collectHackerNews(source) {
  ...
  return items.filter(Boolean).map(x => obs(source, {
    id:x.id,
    title:x.title,
    url:x.url || `https://news.ycombinator.com/item?id=${x.id}`,
    publishedAt:new Date(x.time*1000).toISOString(),
    engagement:(x.score||0)+(x.descendants||0),
    summary:`${x.score||0} points · ${x.descendants||0} comments`
  }));
}
...
async function collectReddit(source) {
  ...
  return (data?.data?.children || []).map(({data:x}) => {
...
async function collectPubMed(source) {
  ...
  return ids.map(id => summary?.result?.[id]).filter(Boolean).map(x => obs(source, {
    id:x.uid,
    title:x.title,
    url:`https://pubmed.ncbi.nlm.nih.gov/${x.uid}/`,
    publishedAt:x.pubdate ? new Date(x.pubdate).toISOString() : new Date().toISOString(),
    summary:(x.fulljournalname||'PubMed') + (x.sortfirstauthor ? ` · ${x.sortfirstauthor}`:'')
  }));
}
...
async function collectUSGS(source) {
  ...
  return (data.features||[]).slice(0,12).map(x => obs(source, {
    id:x.id,
    title:`M${x.properties.mag} earthquake — ${x.properties.place}`,
    url:x.properties.url,
    publishedAt:new Date(x.properties.time).toISOString(),
    engagement:Math.round((x.properties.mag||0)*10),
    summary:`Depth ${x.geometry?.coordinates?.[2] ?? '?'} km · USGS reviewed event`
  }));
}
...
function clusterObservations(observations) {
  ...
  return clusters.map(c=>{
    ...
    return {
      ...
      primaryCategory:c.categories[0],
      score,
