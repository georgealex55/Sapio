...
function getSql(){
  ...
  if(!sqlClient){
    neonFactory=neonFactory||require('@neondatabase/serverless').neon;
    sqlClient=neonFactory(process.env.DATABASE_URL);
  }
  return sqlClient;
}
...
async function loadLatestSnapshot(){
  if(!enabled())return null;
  await ensureSchema();
  const rows=await getSql().query(`SELECT id,generated_at,refresh_after,payload FROM sapio_feed_snapshots ORDER BY generated_at DESC LIMIT 1`);
...
async function recordRenewalRequest(trigger='feed-stale',metadata={}){
  if(!enabled())return null;
  await ensureSchema();
  const rows=await getSql().query(
    `INSERT INTO sapio_timeline_renewals (trigger_type,status,metadata) VALUES ($1,'requested',$2::jsonb) RETURNING id`,
    [trigger,JSON.stringify(metadata)]
  );
  return rows[0]?.id||null;
}
module.exports={enabled,ensureSchema,saveSnapshot,loadLatestSnapshot,recordRenewalRequest};
